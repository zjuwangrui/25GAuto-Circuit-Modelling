 ...
 *  背景 (硬件通路):
 *
 *      AD9910 ── ×4 放大 ──── H(s) ────  →  实际观测点 (示波器)
 *      (差分 Vpp)   (硬件      (二阶低通,
 *                    电路)      DC 增益 5)
 *
 *          H(s) = 5 / (1e-8·s² + 3e-4·s + 1)
 *              固定不变, ωn = 10⁴ rad/s ≈ 1591 Hz,
 *              ζ = 1.5 (过阻尼), 两实极点 ~608 Hz 与 ~4166 Hz.
 *
 *  职责:
...
#define SIGNAL_OUT_AMP_GAIN     5.11f
#endif

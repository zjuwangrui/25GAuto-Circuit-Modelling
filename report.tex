...
M1 和 M2 之间自由切换 (可交替使用); M3 学习完成后落回 IDLE 但保
留了滤波器参数, 用户可以断开测试线接入信号发生器再进 M4. 顶层任务
仅两个: \texttt{panel\_ctrl\_task} (50 ms) 处理屏事件与模式切换,
\texttt{mode\_task} 按当前模式推进 M3/M4 内部流程; ADC/DAC 用定时
器触发, IIR 处理直接在 DMA 完成中断里做, 不进任务调度.
...
\begin{minipage}{0.90\textwidth}
\begin{verbatim}
   信号发生器 --> 装置输入口 (Rin >= 100kΩ) --> ADC (TIM 触发)
                                                     |
                                                     v
                                           IIR biquad (CMSIS-DSP)
                                           [b0, b1, b2, a1, a2]
                                           来自学习结果
                                                     |
                                                     v
...
\begin{enumerate}[leftmargin=2.3em,itemsep=0.35em]
  \item \textbf{已知模型误差。}1~kHz点偏差最大，可能来自4.7~nF与10~nF电容容差、面包板或PCB寄生、OP27实际闭环增益及示波器读数误差。可优先实测电容并筛选，或将后级反馈电阻改为可微调结构；软件控制时使用式\eqref{eq:fitH}或分段查表补偿。
  \item \textbf{M2端点误差。}200~Hz和3~kHz点超限，表明单一增益系数不能覆盖整个频带。

...
比较可得
\begin{equation}
K=5,\qquad \omega_0=10^4~\mathrm{rad/s},\qquad f_0=\frac{\omega_0}{2\pi}=1591.5~\mathrm{Hz},\qquad Q=\frac{1}{3}.
\end{equation}

实际SK电路中，$R_1=R_2=15~\mathrm{k\Omega}$，反馈电容$C_1=4.7~\mathrm{nF}$，对地电容$C_2=10~\mathrm{nF}$，运放接成电压跟随器。其传递函数为
\begin{equation}
H_{\mathrm{SK}}(s)=\frac{1}{R_1R_2C_1C_2s^2+(R_1+R_2)C_2s+1}.
\end{equation}

OP27后级采用反相放大，$R_{\mathrm{in}}=8.2~\mathrm{k\Omega}$，$R_f=40~\mathrm{k\Omega}$，因此
\begin{equation}
A_v=-\frac{R_f}{R_{\mathrm{in}}}=-\frac{40}{8.2}=-4.878.
\end{equation}

忽略运放有限带宽后，实际元件标称值对应的总传递函数为
\begin{equation}
H_{\mathrm{design}}(s)=\frac{-4.878}{1.0575\times10^{-8}s^2+3.000\times10^{-4}s+1}.
\label{eq:designH}
\end{equation}

由式\eqref{eq:targetH}和式\eqref{eq:designH}得到表\ref{tab:paramcompare}。可以看出，一次项系数与目标值相同，二次项系数偏差5.75\%，直流增益偏差$-2.44\%$，元件选取基本合理。

\begin{table}[H]
\centering
\caption{目标模型与实际元件标称值比较}
\label{tab:paramcompare}
\begin{tabular}{lccc}
\toprule
参数 & 目标值 & 实际设计值 & 相对偏差 \\
\midrule
直流增益$|K|$ & 5.000 & 4.878 & $-2.44\%$ \\
一次项系数 & $3.000\times10^{-4}$ & $3.000\times10^{-4}$ & $0.00\%$ \\
二次项系数 & $1.000\times10^{-8}$ & $1.0575\times10^{-8}$ & $+5.75\%$ \\
特征频率$f_0$ & 1591.5~Hz & 1547.7~Hz & $-2.76\%$ \\
品质因数$Q$ & 0.333 & 0.343 & $+2.83\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{DDS放大模块分析}

OPA847采用同相放大结构，反馈电阻$R_{15}=750~\Omega$，下臂电阻$R_{14}=39~\Omega$，闭环增益为
\begin{equation}
G_{\mathrm{amp}}=1+\frac{750}{39}=20.23.
\end{equation}
输入端设置50~$\Omega$终端电阻和33~$\Omega$串联电阻，以减小DDS连接线反射；输出端串联50~$\Omega$电阻，提高驱动同轴线时的稳定性。正负电源各使用6.8~$\mu$F与100~nF并联去耦。

输出3~Vpp、1~MHz正弦波所需的最小压摆率为
\begin{equation}
SR_{\min}=2\pi fV_p=\pi fV_{pp}=9.42~\mathrm{V/\mu s}.
\end{equation}
因此该级必须使用高速运放，并重点控制反馈回路面积、地回流路径和输出负载；若实测高频幅度提前下降，应优先检查终端匹配、供电去耦、PCB寄生和DDS幅度设置，而不能简单归因于1~MHz本身。

\subsection{测频模块分析}

LM339负端接参考电压。由20.3~k$\Omega$与10~k$\Omega$分压可得
\begin{equation}
V_{\mathrm{ref}}=V_{CC}\frac{10}{20.3+10}=0.330V_{CC}.
\end{equation}
当$V_{CC}=5$~V时，$V_{\mathrm{ref}}\approx1.65$~V。比较器输出为开集电极结构，4.7~k$\Omega$上拉与5.6~k$\Omega$下拉使高电平约为
\begin{equation}
V_{OH}=V_{CC}\frac{5.6}{4.7+5.6}=0.544V_{CC}\approx2.72~\mathrm{V},
\end{equation}
可直接送入3.3~V单片机输入口。

输入信号经22~nF隔直电容和100~k$\Omega$串联电阻进入比较器正端，47~k$\Omega$电阻将该点偏置到$V_{\mathrm{ref}}$，3.3~M$\Omega$反馈电阻提供迟滞。按电阻网络估算，5~V供电时上下门限相差约80~mV，可抑制过零附近的噪声重复触发。交流输入等效阻抗约为
\begin{equation}
R_{\mathrm{in,ac}}\approx100~\mathrm{k\Omega}+(47~\mathrm{k\Omega}\parallel3.3~\mathrm{M\Omega})\approx146~\mathrm{k\Omega},
\end{equation}
满足发挥部分输入电阻不小于100~k$\Omega$的要求。由22~nF耦合电容形成的高通拐点约为50~Hz，对1~kHz以上输入影响很小。

主控定时器输入捕获相邻$N$个周期的计数差$\Delta C$，频率计算式为
\begin{equation}
f_{\mathrm{in}}=\frac{Nf_{\mathrm{TIM}}}{\Delta C}.
\end{equation}
对多个周期求平均，可降低比较器边沿抖动和计数器量化误差。

\subsection{十六倍频PLL分析}

CD74HC4046A的VCO输出送入SN74HC161时钟端，计数器$Q_3$端输出为VCO频率的$1/16$，再反馈到4046比较输入端。锁定时两个鉴相信号同频，因此
\begin{equation}
\frac{f_{\mathrm{VCO}}}{16}=f_{\mathrm{in}},\qquad f_{\mathrm{VCO}}=16f_{\mathrm{in}}.
\end{equation}

4046的$R_1=6.04$~k$\Omega$、$R_2=294$~k$\Omega$和$C=1.8$~nF决定VCO中心频率与调节范围；相位比较器输出经47~k$\Omega$和100~nF构成一阶环路滤波，时间常数为
\begin{equation}
\tau=RC=4.7~\mathrm{ms},\qquad f_c\approx\frac{1}{2\pi RC}=33.9~\mathrm{Hz}.
\end{equation}

需要特别说明：SN74HC161的$Q_3$是除16后的反馈信号，十六倍频输出应从4046的VCOOUT节点取得。锁定后$Q_3$与输入同频，而VCOOUT为输入频率的16倍。
...
已知模型原理图如图\ref{fig:known}。输入先经过$15~\mathrm{k\Omega}$、$15~\mathrm{k\Omega}$电阻网络，$4.7~\mathrm{nF}$反馈电容和$10~\mathrm{nF}$对地电容构成单位增益SK二阶低通。NE5532的另一半接成零电平跟随器，避免未使用运放输入悬空。滤波输出经$8.2~\mathrm{k\Omega}$进入OP27反相端，$40~\mathrm{k\Omega}$反馈电阻设置约4.88倍增益；同相端经10~k$\Omega$接地。各电源引脚附近设置100~nF去耦。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{fig_known_model.png}
\caption{已知模型电路原理图}
\label{fig:known}
\end{figure}
...
元件 & 参数 & 作用 \\
\midrule
$R_1,R_2$ & 15~k$\Omega$ & SK低通两级串联电阻 \\
$C_1$ & 4.7~nF & 输出至一级节点的反馈电容 \\
$C_2$ & 10~nF & 二级节点对地电容 \\
NE5532 & 电压跟随 & 构成单位增益SK级 \\
$R_{11}$ & 8.2~k$\Omega$ & OP27反相输入电阻 \\
$R_5$ & 40~k$\Omega$ & OP27反馈电阻 \\
\bottomrule
\end{tabular}
\end{table}
...
DDS放大模块如图\ref{fig:ddsamp}。OPA847采用约20.23倍同相放大。输入端50~$\Omega$终端可兼容同轴线和DDS模块，33~$\Omega$串联电阻隔离输入寄生；输出端50~$\Omega$电阻用于隔离电容性负载。反馈电阻应靠近运放引脚布置，正负电源的6.8~$\mu$F和100~nF去耦电容分别承担低频储能和高频旁路。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{fig_dds_amp.png}
\caption{DDS输出放大模块原理图}
\label{fig:ddsamp}
\end{figure}
...
测频模块如图\ref{fig:freq}。输入经22~nF隔直后进入LM339，比较器负端固定在$V_{\mathrm{ref}}$，正端由47~k$\Omega$偏置并由3.3~M$\Omega$形成迟滞。输出端使用4.7~k$\Omega$上拉和5.6~k$\Omega$下拉，使逻辑高电平低于3.3~V。其余三个未使用比较器的输入端固定到确定电位，避免输入悬空产生不确定翻转。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{fig_freq_measure.png}
\caption{LM339测频整形模块原理图}
\label{fig:freq}
\end{figure}

\subsection{十六倍频PLL模块}

十六倍频模块如图\ref{fig:pll}。输入方波接CD74HC4046A的SIGNAL IN，VCOOUT接SN74HC161的CP端。计数器并行数据端接低电平，计数允许端置有效，$Q_3$输出经连接端口返回4046的COMP IN，闭环后实现除16反馈。4046的PC2OUT经47~k$\Omega$、100~nF低通后控制VCOIN。模块输出应从VCOOUT节点引出，$Q_3$只作为反馈与锁定观察点。

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{fig_pll_x16.png}
\caption{CD74HC4046A与SN74HC161构成的十六倍频PLL}
\label{fig:pll}
\end{figure}
...
表\ref{tab:knownmeas}为现有实测记录。理论值由题目目标传递函数式\eqref{eq:targetH}计算，而不是由实际元件标称值计算。

\begin{table}[H]
\centering
\caption{已知模型输出实测（输入1~Vpp正弦）}
\label{tab:knownmeas}
\begin{tabular}{cccc}
\toprule
频率 & 理论输出/Vpp & 实测输出/Vpp & 相对误差 \\
\midrule
100~Hz  & 4.932 & 4.880 & 1.05\% \\
500~Hz  & 3.834 & 3.540 & 7.67\% \\
1~kHz   & 2.526 & 2.240 & 11.32\% \\
1.5~kHz & 1.767 & 1.600 & 9.45\% \\
2~kHz   & 1.311 & 1.200 & 8.47\% \\
2.5~kHz & 1.013 & 0.960 & 5.23\% \\
3~kHz   & 0.806 & 0.800 & 0.74\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{known_model_response.png}
\caption{目标曲线、实际元件设计曲线与实测点比较}
\label{fig:knowncurve}
\end{figure}

除1~kHz测点外，其余测点相对误差均不超过10\%；1~kHz点相对误差为11.32\%，超过限值1.32个百分点。实测曲线仍保持二阶低通形状。将七个测点拟合到式\eqref{eq:fitH}后，相对均方根拟合误差约为0.49\%，说明主要偏差可用板级参数标定进行补偿。
...
本设计围绕 G 题要求, 以 STM32F103VE 为主控, AD9910 DDS 为激励源,
辅以片内 ADC/DAC 与大彩串口屏, 完成了信号输出 (M1)、已知模型控制
(M2)、未知电路学习 (M3)、推理生成信号 (M4) 四种工作模式的完整软
硬件框架. 已知模型电路采用 SK 二阶低通结构, 后级独立放大补偿直流
增益; 反算模块基于对实测响应的二阶拟合, 在软件层补偿板级容差; 学
习模块用 DDS 数字扫频加 Goertzel 单点 DFT 建立幅频响应, 结合两端
特征判据与 Gauss-Newton 拟合完成滤波类型识别与参数估计; 推理模块
经双线性变换离散化学到的传递函数, 在 ADC 转换中断内实时跑二阶
biquad, DAC 同一定时器触发输出.

基本要求部分的四项功能均已实现并测得实测数据: 基本要求 (1) 中已
知模型电路的幅频响应实测除 1 kHz 单点略超 10\% 外, 其余测点均在
限值以内; 基本要求 (2) 的 DDS 输出模式在 140 kHz 以内可满足 Vpp
大于 3 V, 频率相对误差远低于 5\%; 基本要求 (3)(4) 的已知模型控
制模式经反算后, 观测点峰峰值与设定值的相对误差满足题目 5\% 要求.

本设计发挥部分 (未知电路学习模式 M3、推理
生成信号模式 M4) 的功能尚未进行实测验证.，无测试数据
